...
begin
	...
	using PETLION 		# Electrochemical simulation model for Li-ion batteries 
end

# ╔═╡ af7c74fd-eff2-4df1-a936-1646d9c8e563
ThreeColumn(md"`SAE_overview_SC_bat.jl`", md"[![](https://img.shields.io/badge/GitHub_URL-notebook-C09107)](https://github.com/Ricardo-Luis/notebooks/blob/main/SAE/SAE_overview_SC_bat.jl)", md"`Last update: 16·03·2023`")

# ╔═╡ 38935ff3-691a-42d9-b8b1-cdb6ed3fcf42
md"""
 ---
"""

# ╔═╡ 120f7972-0f12-4a99-b3ef-e1a7b279d2b2
md"""
...
![](https://i.imgur.com/RljiN8v.png)

>Source: [EASE - European Association for Storage of Energy](https://ease-storage.eu/).
...
Example of a Ragone plot of energy storage technologies. This plot often times is presented with gravimetric quantity units $$\rm (W/kg \:\:\it vs.\:\rm Wh/kg)$$, or volumetric quantity units $$\rm (W/l \:\:\it vs.\:\rm Wh/l)$$.

![](https://i.imgur.com/bWMIJ0h.png)

> Source: [ESA -  Energy Storage Association](https://energystorage.org/)
...
>Source: EPCI - European Passive Components Institute, [^1].
...
# ╔═╡ 1d2efeb5-dd43-477f-9575-52bbf57ec7c7
md"""
## Reference book
"""
...
## Lead-acid batteries
In 1859, the French physician Gaston Planté invented the first rechargeable battery based on lead acid.
"""
...
# ╔═╡ a631acf5-4515-4a8f-95c1-20371d4be503
Lead_Acid_bat = ce"Pb + PbO2 + H2SO4 → PbSO4 + H2O";  

# ╔═╡ 7ff28faa-75cf-4ae4-8022-823a0706c3d1
balance(Lead_Acid_bat)
...
![](https://i.imgur.com/VlfZMKi.png)

> Source: ESA - Energy Storage Assocation, [^2].
...
begin
	apps = ["Automotive (starter battery)", "Motorcycle (starter battery)", "UPS", "Transport vehicles (including forklifts)", "Electrical bicycle", "Other vehicles", "Other applications"]
	values = [52, 6, 10, 9, 9, 6, 10]
	
	pie(apps, values)

	annotate!(-0.6, 0.45, text("$(values[1])%", 10))
	annotate!(0.75, -0.25, text("$(values[7])%", 10))
	annotate!(0.55, -0.55, text("$(values[6])%", 10))
	annotate!(0.25, -0.75, text("$(values[5])%", 10))
	annotate!(-0.19, -0.75, text("$(values[4])%", 10))
	annotate!(-0.55, -0.55, text("$(values[3])%", 10))
	annotate!(-0.75, -0.2, text("$(values[2])%", 10))
end
...
The development of lithium-ion battery owes to work of M. Stanley Whittingham for discovered the concept of intercalation electrodes in the 1970s and created the first rechargeable lithium-ion battery,  although it suffered from safety issues and was never commercialized. John B. Goodenough expanded on this work in 1980 by using lithium cobalt oxide as a cathode. The first prototype of the modern Li-ion battery, which uses a carbonaceous anode was developed by Akira Yoshino in 1985, which was commercialized by a Sony and Asahi Kasei in 1991, [^4].

In 2019 John B. Goodenough, M. Stanley Whittingham  and Akira Yoshino were awarded the 2019 Nobel Prize in Chemistry "for the development of lithium-ion batteries". 
...
md"""
\
**Schematic diagram of the lithium-ion battery**, [^5]
...
md"""
### 💻 Li-ion battery technologies: radar chart 
\
"""

# ╔═╡ 46b5fb35-eb2c-4374-af49-efa859610480
@bind batteries MultiSelect(["Lithium Cobalt Oxide", "Lithium Manganese Oxide", "Lithium Iron Phosphate", "Lithium Nickel Manganese Cobalt Oxide - NMC", "Lithium Nickel Cobalt Aluminium Oxide - NCA", "Lithium Titanate"])

# ╔═╡ dc32100f-9985-435e-939b-fb4931d0321a
md"""
> Source: Li-ion technologies comparision data extracted from [^6].
"""

# ╔═╡ b6b52384-f52a-4135-b09b-22ffdaac1312
md"""
📃  **Optional:** unhide the next 2 `Julia` code cells to follow the radar graph build!
"""

# ╔═╡ e794c4da-e950-4c2f-92dc-c624f9cf47d5
begin
	Cobalt = [2, 3, 2, 4, 3, 2, 2]
	Manganese = [3, 2, 3, 3, 3, 2, 3]
	Iron = [3, 3, 4, 2, 3, 4, 3]
	NMC = [3, 3, 3, 4, 3, 3, 3]
	NCA = [4, 3, 2, 4, 2, 4, 4]
	Titanate = [3, 4, 4, 2, 1, 4, 3]

	LiB = zero(rand(6,7)) .+ NaN
	
	if "Lithium Cobalt Oxide" ∈ batteries
		LiB[1,:].= Cobalt
	else
		LiB[1,:].= NaN
	end
	if "Lithium Manganese Oxide" ∈ batteries
		LiB[2,:].= Manganese
	else
		LiB[2,:].= NaN
	end
	if "Lithium Iron Phosphate" ∈ batteries
		LiB[3,:].= Iron
	else
		LiB[3,:].= NaN
	end
	if "Lithium Nickel Manganese Cobalt Oxide - NMC" ∈ batteries
		LiB[4,:].= NMC
	else
		LiB[4,:].= NaN
	end
	if "Lithium Nickel Cobalt Aluminium Oxide - NCA" ∈ batteries
		LiB[5,:].= NCA
	else
		LiB[5,:].= NaN
	end
	if "Lithium Titanate" ∈ batteries
		LiB[6,:].= Titanate
	else
		LiB[6,:].= NaN
	end
...
# ╔═╡ 85d2faed-7acc-4ab4-b950-b1c71b804d53
FindNaN=isnan.(LiB[:,1]);

# ╔═╡ 5f61a216-deac-40b1-a7b9-445f4e5e087b
begin
	# PLOT:
	n=6
	#[SP, Perf, Safety, SE, cost, Life, SP]
	
	labels = "Performance", "Safety", "Specific energy", "Cost", "Life span", "Specific power"
	justifyme(θ) = (0≤θ<π/2 || 3π/2<θ≤2π) ? :left : (π/2<θ<3π/2) ? :right : :center
	θ = LinRange(0, 2pi, n+1)
	
	z = 1.3*exp.(im*2π*(1:(n))/n)
	colors=[:blue :red :brown :orange :grey :green]
	Chemistry=["LiCoO₂" "LiMn₂O₄" "LiFePO₄" "LiNiMnCoO₂" "LiNiCoAlO₂" "Li₄Ti₅O₁₂"]

	p=plot(θ, zero(rand(1,7))', proj=:polar, xaxis=false, margin=10Plots.mm, label=false)
	
	for i in 1:6
		if FindNaN[i,1]==false
			plot!(θ, LiB[i,:], proj=:polar, ms=2, m=:o, 
			c=colors[1,i], fill=(true,colors[1,i]), 
			fa=0.4, lims=(0,4), xaxis=false, margin=10Plots.mm, 
			label=Chemistry[1,i], legend=:bottomright)
		end
	end
		
	p
	annotate!(real.(z), imag.(z), 
		text.(labels,10,justifyme.(θ[1:n]),"Computer Modern"))
end
...
> Source : Example of a custom lithium-ion battery pack using 18650 type cells. Extracted from: [Large Power](https://www.large.net/)
...
md"""
### 💻🔗[Series/parallel arrangement of battery cells](http://liionbms.com/php/wp_series_parallel.php)
"""
...
> Source: [Battery Management System - CellWatt](https://cellwatt.com/index.php/component/k2/item/13-battery-management-system)
...
md"""
### 💻🔗[Cell balance visualizer](http://liionbms.com/balance/)
"""
...
begin
	...
	p4 = plot(sol, :T)
	p5 = plot(sol, :P)
	plot(p1, p5, p2, p4, p3, layout=(3,2), size=(800,700))
end

# ╔═╡ c2929125-83a1-449f-8ce8-bff849996c57
md"""
!!! note
	In our classes, we will use equivalent circuit models for numerical simulation of secondary batteries, [^8].
"""

# ╔═╡ eeae622e-a8c1-4445-92b9-95555f894356
md"""
### Reference books
"""
...
\
- Fast charging solutions (source: Configuration of the fast EV charging station including stationary energy storage system, [^9] ) 
...
- Commercial Site 
![](https://i.imgur.com/QbzaaBw.jpg)

- Service Station
![](https://i.imgur.com/WFezO5q.jpg)

- Data Centre & Hydrogen Service Station
...
\
[**Slides** from Petar J. Grbović book: Ultra-Capacitors in Power Conversion: Applications, Analysis and Design from Theory to Practice](https://slideplayer.com/slide/5977050/)
...
[Lithium-Ion Battery Inventor Introduces New Technology for Fast-Charging, Noncombustible Batteries - UT News](https://news.utexas.edu/2017/02/28/goodenough-introduces-new-battery-technology/)
...
[**Slides** from the Davide Andrea books: Lithium-ion Batteries and Applications, Vol.1 & 2](https://liionbook.com/)
...
md"""
[^1]: [European Passive Components Institute (EPCI), Supercapacitors, knowledge-blog: an educational & information blog with passive components news and technical information.](https://passive-components.eu/supercapacitors/)

[^2]: [Energy Storage Association (ESA), Lead Batteries section](https://energystorage.org/why-energy-storage/technologies/solid-electrode-batteries/)

[^3]: Jung, J., et all, Lead-acid battery technologies: fundamentals, materials and applications”, CRC Press, 2016.

[^4]: [Lithium-ion battery - Wikipedia](https://en.wikipedia.org/wiki/Lithium-ion_battery)

[^5]: Tidblad, A.A, Berg, H., et. all, “Batteries – Present and future challenges”, Swedish Hybrid Vehicle Centre, 10-2015.

[^6]: [Battery University™, Types of Lithium-ion, a free educational website about battery information](https://batteryuniversity.com/article/bu-205-types-of-lithium-ion)

[^7]:
$(DOI("10.1002/aenm.202201506"))

[^8]: Rahmoun, A., Biechl, H., [Modelling of Li-ion batteries using equivalent circuit diagrams](http://pe.org.pl/articles/2012/7b/40.pdf), Przeglad Elektrotechniczny. 88. pgs. 152-156, University of Applied Sciences Kempten, 2012. 
...
**Time to first plot**: up to 11 min.  (Be patient!)\
...
md"""
Documentation of used `Julia` packages: [PlutoUI](https://github.com/JuliaPluto/PlutoUI.jl), [PlutoTeachingTools](https://github.com/JuliaPluto/PlutoTeachingTools.jl), [ShortCodes](https://github.com/hellemo/ShortCodes.jl), [Plots](https://docs.juliaplots.org/stable/), [ChemEquations](https://zlatanvasovic.github.io/ChemEquations.jl/stable/), [PETLION](https://github.com/MarcBerliner/PETLION.jl).
...
# ╔═╡ 07421846-ae64-439d-8192-12e53e39bef4
...
PETLION = "5e0a28e4-193c-47fa-bbb8-9c901cc1ac2c"
...
julia_version = "1.8.4"
manifest_format = "2.0"
project_hash = "a49f31677882d930064992fbafc86009e4203677"
...
[[deps.PETLION]]
deps = ["BSON", "Dierckx", "GeneralizedGenerated", "IfElse", "InteractiveUtils", "KLU", "LinearAlgebra", "OrderedCollections", "PkgVersion", "ProgressMeter", "RecipesBase", "RecursiveArrayTools", "SHA", "SciMLBase", "SparseArrays", "SparseDiffTools", "SpecialFunctions", "StatsBase", "SuiteSparse", "Sundials", "Symbolics"]
git-tree-sha1 = "06cf5420ab919c60ec38c868ec26b02db661bd42"
